...
@Synchronized
def profileStartWin(port) {
	println "Starting profiling " + port
	command = "start.bat  localhost " + port + ""
	println command 
	command.execute() 
}

@Synchronized
def profileOutFileWin(port) {
	def outFile
	def command
	outFile = "c:\\temp\\profile." + port + ".out" 
	command =   "file.bat localhost "+port+" " + outFile + "  "
...
@Synchronized
def profileStopWin(port) {
	println "Stopping profiling" + port 
	
	command="finish.bat localhost " +port
	command.execute() 
...
@Synchronized
def profileDataOutWin(addr,port)
{	
	def jmxPort = -1  // portMapping["jmx"]
	def jipPort = port 
	def folder
	def fName = "profile." + jipPort + ".out.txt"
...
def outWin(addr,port)
{

	
	profileStartWin( port )
//  sleep 30s
	Thread.sleep(10 * 1000)
	//pause(10)
	profileOutFileWin(port) 
	profileStopWin(port) 
//	pause(5)
	this.sleep( 10 * 1000 ) 
//	pause(5)
	profileDataOutWin(addr,port)
...
jipAddress = [] 
jipPortRanges.split(",").each{ portRange ->
	address = "127.0.0.1"
	port = -1 
	
	if (portRange.contains(":")){
		elems = portRange.split(":")
		address = elems[0] 
		port = elems[1].toInteger()
	}else {
		port = portRange.toInteger()
	}

	
	jipAddress.add( [ address ,port ] ) 
}

println "Creatintg pool"
def pool =  Executors.newFixedThreadPool(5)

jipAddress.each {  details -> 
	addr = details[0]
	port = details[1] 
